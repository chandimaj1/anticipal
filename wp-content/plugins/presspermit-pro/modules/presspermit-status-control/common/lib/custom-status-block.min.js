...
var sideEffectL10nManipulation = function sideEffectL10nManipulation(status) {
    ...
    if (node) {
      if (statusLabel && ('publish' != status)) {
        ...
        if ($('#ppcs_save_draft_label').filter(':visible').length) {
          ...
          $('#ppcs_save_draft_label').html(__('Save as %s').replace('%s', statusLabel)).css('left', spanPosLeft + 'px');
        }
      }
    }
  //}, 100);
  ...
  if (('publish' != status) && (-1 == PPCustomStatuses.publishedStatuses.indexOf(status))) {
    ...
    if (!$('div.publishpress-extended-post-status-published').length) {
      $('div.publishpress-extended-post-status select').after('<div class="publishpress-extended-post-status-published" style="display:none;text-align:right;padding-right:5px">' + __('Published') + '</div>');
    }
  ...
  if (-1 == PPCustomStatuses.publishedStatuses.indexOf(status)) {
    $('div.publishpress-extended-post-status-note').html(__("This will override all status settings above.", 'publishpress')).show();
  } else {
    $('div.publishpress-extended-post-status-note').html(__('To select a workflow status, switch to Draft first.', 'publishpress')).show();
  }
};
...
var PPCustomPostStatusInfo = function PPCustomPostStatusInfo(_ref) {
  ...
  }, React.createElement("h4", null, __('Post Status', 'publishpress')), React.createElement(SelectControl, {
    label: "",
    value: status,
